...
module.exports = context => {
  ...
  if ((context.hook === 'after_platform_add' && context.cmdLine.includes('platform add')) ||
    ...
    getPlatformVersionsFromFileSystem(context, projectRoot).then(platformVersions => {
      ...
      // ----------------------------------------------------------------------
      // Cordova iOS 8+ always creates the Xcode project as "App".
      // Older versions still use the application name.
      // ----------------------------------------------------------------------
      if (semver.gte(iosPlatformVersion, '8.0.0')) {
        projectName = 'App';
      } else {
        projectName = config.name();
      }

      // Fallback in case a custom project still uses the old layout.
      if (!fs.existsSync(path.join(platformPath, projectName + '.xcodeproj'))) {
        projectName = config.name();
      }
...
const getPlatformVersionsFromFileSystem = (context, projectRoot) => {
  const cordovaUtil = context.requireCordovaModule(
